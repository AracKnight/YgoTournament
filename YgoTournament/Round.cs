...
namespace YgoTournament
{
    ...
    public class Round
    {
        ...
        {
            get { return _tables.All(tuple => tuple.Value.IsFinished); }
        }
        ...
        public Round(List<Player> players, int nr)
        {
            ...
            var comparer =
                new PlayerComparer(); //sort players by their current tiebreakers then match first with second and so on
        ...
        public void ReportWin(Player p)
        {
            ...
            foreach (var tuple in
                     _tables.Where(tuple =>
                         tuple.Value.Player1 == p || tuple.Value.Player2 == p)) //search match with the given player
            {
                match = tuple.Value;
            }
        ...
        /// <summary>
        /// Reports a draw for a given table number
        /// </summary>
        /// <param name="tableNr">Number of the table to report the draw on</param>
        /// <exception cref="ArgumentException">Throws exception if no match was found for the given table number</exception>
        public void ReportDraw(int tableNr)
        {
            Match match;
            try
            {
                match = _tables[tableNr];
            }
            catch (KeyNotFoundException)
            {
                throw new ArgumentException("No match was found for the given table number");
            }
        ...
        /// <exception cref="ArgumentException">Throws exception if no match is found at the given table</exception>
        public void ReportDoubleLoss(int tableNr)
        {
            ...
            {
                throw new ArgumentException("No match found for given table number.");
            }

            match.ReportDoubleLoss(NumberOfRound);
        }
        ...
        {
            return _tables.Aggregate($"",
                (current, tuple) =>
                    current + $"Table {tuple.Key}:\t{tuple.Value.Player1.Name}\tvs.\t{tuple.Value.Player2.Name}\n");
        }
